(function () {
  ...
  const importBase = decodedPathname === "/admin" || decodedPathname.startsWith("/admin/") ? "/admin" : "";
  ...
  let sanityFetchDisabled = false;

  async function fetchSanityData(query, fallback) {
    if (sanityFetchDisabled) {
      return fallback;
    }

    try {
      // Use the CDN API endpoint which has better CORS support
      const url = `https://${sanityProjectId}.apicdn.sanity.io/v${sanityApiVersion}/data/query/${sanityDataset}?query=${encodeURIComponent(query)}`;
  ...
  window.addEventListener("layout:ready", initializeHeader);

...
function isAddToCartLabel(text) {
  const label = normalizeLabelText(text);
  return /(?:^|\b)(add to cart|ajouter au panier|add to basket|ajouter au chariot)(?:\b|$)/.test(label);
}

function isFavoriteLabel(text) {
  const label = normalizeLabelText(text);
  return /(?:^|\b)(add to favorites|saved to favorites|saved to favorite|favorite|favori|favoris|ajouter aux favoris|ajouté aux favoris|enregistré dans les favoris|supprimer des favoris)(?:\b|$)/.test(label);
}

function getFavoriteLabelSet(text) {
  const label = normalizeLabelText(text);
  if (label.includes("ajouter aux favoris")) {
    return { base: "Ajouter aux favoris", saved: "Ajouté aux favoris" };
  }
  if (label.includes("ajouté aux favoris") || label.includes("enregistré dans les favoris")) {
    return { base: "Ajouter aux favoris", saved: "Ajouté aux favoris" };
  }
  if (label.includes("favoris")) {
    return { base: "Favoris", saved: "Favoris enregistrés" };
  }
  if (label.includes("favori")) {
    return { base: "Favori", saved: "Favori enregistré" };
  }
  if (label.includes("saved to favorites")) {
    return { base: "Add to favorites", saved: "Saved to favorites" };
  }
  if (label.includes("saved to favorite")) {
    return { base: "Favorite", saved: "Saved to favorite" };
  }
  if (label.includes("add to favorites")) {
    return { base: "Add to favorites", saved: "Saved to favorites" };
  }
...
export function extractProductContext(trigger) {
  ...
  // If trigger itself has explicit product data attributes, use it directly
  if (trigger.dataset) {
    const hasExplicitProductData = trigger.dataset.productId || trigger.dataset.productTitle || trigger.dataset.productHref;
    if (hasExplicitProductData) {
      return readDatasetProduct(trigger);
    }
  }
...
export function hydrateProductDataAttributes(root = document) {
  ...
  actionElements.forEach((element) => {
    ...
    const isAddToCart = isAddToCartLabel(text);
    const isFavoriteTrigger =
      element.matches("[data-favorite-toggle]") ||
      isFavoriteLabel(favoriteText) ||
      isFavoriteLabel(text);
...
export function bindAddToCartButtons(root = document, options = {}) {
  ...
  scope.querySelectorAll('button, a, [data-commerce-action="add-to-cart"]').forEach((element) => {
    ...
    const text = getLabelText(element);
    if (!isAddToCartLabel(text) && element.dataset.commerceAction !== "add-to-cart") return;

    const labelNode = element.querySelector(".rfrdb") || element;
    const originalText = getLabelText(labelNode) || "Add to cart";
...
export function bindFavoriteToggles(root = document, options = {}) {
  ...
  scope.querySelectorAll("[data-favorite-toggle], button, a").forEach((element) => {
    ...
    let label = element.querySelector(".rfrdb");
    if (!label && element.matches("[data-favorite-toggle]")) {
      label = document.createElement("span");
      label.className = "rfrdb";
    ...
    if (!isFavoriteLabel(text) && !element.matches("[data-favorite-toggle]")) {
      return;
    }
    ...
    const favoriteLabels = getFavoriteLabelSet(text);
    const baseLabel = favoriteLabels.base;
    const savedLabel = favoriteLabels.saved;
    const renderState = () => {
      const saved = isFavorite(product.product_id);
      label.textContent = saved ? savedLabel : baseLabel;
